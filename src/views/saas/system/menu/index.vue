...
<script>
  import commonTree from '@/components/saas/CommonTree.vue'
    ...
    computed: {
      menuComponent() {
        ...
        if (this.menu.path && this.menu.path !== '/') {
          ...
          comp = `组件路径：src/views/saas/index.vue`
        }
        return comp
      }
    },
    ...
    methods: {
      ...
      computedComponent() {
        ...
        if (isUrl) {
          ...
          if (this.menu.path) {
            this.menu.component = `saas${this.menu.path}/index`
          } else {
            this.menu.component = `saas/index`
          }
        }
      },
      initMenuTree() {
